...
def train(my_model, optimizer, loss_fn, cnn_dataloaders, device, scaler):  # cnn_dataloaders = cnn_dataloaders['train']
    my_model.train()
    train_loss = 0.0
    for data, targets, descriptors in cnn_dataloaders['train']:
    ...
    avg_train_loss = train_loss / len(cnn_dataloaders['train'].dataset)
    return avg_train_loss


# Validation function with AMP (autocast only)
def validate(my_model, loss_fn, cnn_dataloaders, device):  # cnn_dataloaders = cnn_dataloaders['val']
    my_model.eval()
    val_loss = 0.0
    with torch.no_grad():
        for data, targets, descriptors in cnn_dataloaders['val']:
            data, targets, descriptors = data.to(device), targets.to(device), descriptors.to(device)  # to cuda
            with autocast():
                outputs = my_model(data, descriptors)
                outputs = outputs[:, 0]
    ...
    avg_val_loss = val_loss / len(cnn_dataloaders['val'].dataset)
    return avg_val_loss


# Test function using various metrics with AMP (autocast only)
def test(my_model, cnn_dataloaders, device, plot_save_folder, model_name,
    ...
    with torch.no_grad():
        for data, targets, descriptors in cnn_dataloaders['test']:
            data, targets, descriptors = data.to(device), targets.to(device), descriptors.to(device)  # to cuda
