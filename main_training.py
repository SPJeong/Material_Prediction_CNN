...
descriptor_df_columns_list = file_raw.columns[start_column_index_desc:end_column_index_desc + 1].tolist()
...
for i, target_name in tqdm(enumerate(Y_total_list), total=len(Y_total_list)):
    ...
    # load cnn_dataloaders
    cnn_dataloaders = my_utils.prepare_cnn_data_loaders(combined_df=concat_df_raw,
    ...
    ''' 
    # cnn_dataloaders check
    for data in cnn_dataloaders["train"]:
        x, y, desc = data
    ...
    # CNN_model, loss fn and optimizer define
    my_model = model.CNN()  # (1, 64, 64)
    my_model.to(device)
    loss_fn = nn.MSELoss()
    ...
    for epoch in range(0, epochs):
        ...
        train_loss = train(my_model, optimizer, loss_fn, cnn_dataloaders, device,
                           scaler)  # cnn_dataloaders = cnn_dataloaders['train']
        val_loss = validate(my_model, loss_fn, cnn_dataloaders, device)  # cnn_dataloaders = cnn_dataloaders['val']
    ...
    final_metrics = test(my_model, cnn_dataloaders, device, plot_save_folder, model_name,
                         target_name)  # cnn_dataloaders = cnn_dataloaders['test']
